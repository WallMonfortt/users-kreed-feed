...
import { updateGralInfoAddress } from '../../../actions/user';
import { useForm } from '../../../hooks/useForm';

export const GralAddress = () => {
  const {address} = useSelector(state => state.info)
  ...
  const [formValues, handleInputChange] = useForm({
    Calle_o_Avenida: address.Calle_o_Avenida,
    Número_exterior:address.Número_exterior,
    Número_interior: address.Número_interior,
    Código_Postal: address.Código_Postal,
    Colonia_o_Urbanización:address.Colonia_o_Urbanización,
    Ciudad_o_Población:address.Ciudad_o_Población,
    Entidad_Federativa_o_Estado:address.Entidad_Federativa_o_Estado,
    País:address.País
  });
  ...
  const handleSubmit = (e) =>{
    e.preventDefault();
    if (!errors) {
      dispatch(updateGralInfoAddress(formValues));
      setMessaggeError('')
  ...
  return (
    <div>
     {!gralInfoStatus && <h3 className="alerts">Please comple your Address information</h3>}
